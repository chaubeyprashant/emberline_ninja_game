...
{
    public enum RigPose { Idle, Run, Strike1, Strike2, Cleave, Windup, Hurt, Dash, Dead }

    /// <summary>
    /// Articulated ninja built from primitives at runtime and animated procedurally —
    /// no models, no animation clips. Joint hierarchy: pelvis → torso → head/arms,
    /// pelvis → legs; sword (with trail) in the right hand. Owners drive it with
    ...
    public class NinjaRig : MonoBehaviour
    {
        ...
        public void PlayOneShot(RigPose pose, float duration)
        {
            _oneActive = true;
            _onePose = pose;
            _oneT = 0f;
        ...
        public void ForcePose(RigPose pose, float phase)
        {
        ...
        public void Flash() => _flashT = 0.13f;

        public void SetGhostAlpha(float a)
        {
            if (!ghost) return;
        ...
        private void LateUpdate()
        {
            ...
            if (_trail != null)
                _trail.emitting = (pose is RigPose.Strike1 or RigPose.Strike2 or RigPose.Cleave)
